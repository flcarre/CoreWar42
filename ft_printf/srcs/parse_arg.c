...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/02/21 17:37:12 by wahasni           #+#    #+#             */
...
void	get_min_length(t_printf *pf)
{
	if (*pf->format >= 49 && *pf->format <= 57)
		pf->min_length = ft_atoip(pf->format);
	if (*pf->format == '*')
	{
		pf->format++;
...
void	get_precision(t_printf *pf)
{
	...
	{
		pf->format++;
		pf->precision = ft_atoip(pf->format) ? ft_atoip(pf->format) : -1;
		if (*pf->format == '*')
		{
			pf->format++;
